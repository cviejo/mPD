...
public:
	...
	void addPath(const std::string& path, const std::string& name = "") {
		...
		std::vector<Path>::iterator iter = std::find_if(
		   paths.begin(), paths.end(), [&path](Path const& p) { return p.path == path; });
		if (iter == paths.end()) {
	...
	void removePathByName(const std::string& name) {
		mutex.lock();
		std::vector<Path>::iterator iter = std::find_if(
		   paths.begin(), paths.end(), [&name](Path const& p) { return p.name == name; });
		if (iter != paths.end()) {
			paths.erase(iter);
		}
		mutex.unlock();
	...
	void setRemoveDeletedPaths(bool remove) { removeDeleted = remove; }
	...
	bool waitingEvents() { return !queue.empty(); }
