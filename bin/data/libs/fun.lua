...
local wrap = function(gen, param, state)
	return setmetatable({gen = gen, param = param, state = state}, iterator_mt),
	       param, state
end
exports.wrap = wrap
...
local each = function(fun, gen, param, state)
	repeat state = call_if_not_empty(fun, gen(param, state)) until state == nil
end
...
local all = function(fun, gen_x, param_x, state_x)
	local r
	repeat state_x, r = call_if_not_empty(fun, gen_x(param_x, state_x)) until state_x ==
					nil or not r
	return state_x == nil
end
...
local min = function(gen, param, state)
	...
	for _, r in gen, param, state do m = cmp(m, r) end
	return m
end
...
local min_by = function(cmp, gen_x, param_x, state_x)
	...
	for _, r in gen_x, param_x, state_x do m = cmp(m, r) end
	return m
end
...
local numargs = function(...)
	...
	if n >= 3 then
		-- Fix last argument
		local it = select(n - 2, ...)
		if type(it) == 'table' and getmetatable(it) == iterator_mt and it.param ==
						select(n - 1, ...) and it.state == select(n, ...) then return n - 2 end
	end
	return n
end
