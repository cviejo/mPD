...
local cordMesh = require('gui/pd/cord-mesh')
local grid = require('gui/pd/grid')
local drawItem = require('gui/pd/draw-item')
local Viewport = require('gui/pd/viewport')
local withViewport = require('gui/pd/with-viewport')
local pd = require('pd')
local events = require('events')

local map = F.map

local pointsToMesh, pointsToPath = ofx.pointsToMesh, ofx.pointsToPath
...
return function(id, position)
	...
	M.message = function(msg)
		...
		elseif not msg.id then
			log(msg)
			log(red("no id"))
		elseif hasTag('cord', msg) and msg.params.width == 1 then
